...
var (
	...
	QueryGetGroupListCount = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-01",
		Query: `SELECT COUNT(*) as total FROM "GROUP" WHERE SERVER_ID = $1`,
	}

	// QueryGetGroupList is the query to get groups with pagination.
	QueryGetGroupList = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-02",
		Query: `SELECT GROUP_ID, OU_ID, NAME, DESCRIPTION FROM "GROUP" WHERE SERVER_ID = $3 ORDER BY NAME LIMIT $1 OFFSET $2`,
	}

	// QueryCreateGroup is the query to create a new group.
	QueryCreateGroup = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-03",
		Query: `INSERT INTO "GROUP" (GROUP_ID, OU_ID, NAME, DESCRIPTION, SERVER_ID) VALUES ($1, $2, $3, $4, $5)`,
	}

	// QueryGetGroupByID is the query to get a group by id.
	QueryGetGroupByID = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-04",
		Query: `SELECT GROUP_ID, OU_ID, NAME, DESCRIPTION FROM "GROUP" WHERE GROUP_ID = $1 AND SERVER_ID = $2`,
	}

	// QueryGetGroupMembers is the query to get members assigned to a group.
	QueryGetGroupMembers = dbmodel.DBQuery{
		ID: "GRQ-GROUP_MGT-05",
		Query: `SELECT MEMBER_ID, MEMBER_TYPE FROM GROUP_MEMBER_REFERENCE WHERE GROUP_ID = $1 AND SERVER_ID = $4 ` +
	...
	QueryGetGroupMemberCount = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-06",
		Query: `SELECT COUNT(*) as total FROM GROUP_MEMBER_REFERENCE WHERE GROUP_ID = $1 AND SERVER_ID = $2`,
	}

	// QueryUpdateGroup is the query to update a group.
	QueryUpdateGroup = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-07",
		Query: `UPDATE "GROUP" SET OU_ID = $2, NAME = $3, DESCRIPTION = $4 WHERE GROUP_ID = $1 AND SERVER_ID = $5`,
	}

	// QueryDeleteGroup is the query to delete a group.
	QueryDeleteGroup = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-08",
		Query: `DELETE FROM "GROUP" WHERE GROUP_ID = $1 AND SERVER_ID = $2`,
	}

	// QueryDeleteGroupMembers is the query to delete all members assigned to a group.
	QueryDeleteGroupMembers = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-09",
		Query: `DELETE FROM GROUP_MEMBER_REFERENCE WHERE GROUP_ID = $1 AND SERVER_ID = $2`,
	}

	// QueryAddMemberToGroup is the query to assign member to a group.
	QueryAddMemberToGroup = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-10",
		Query: `INSERT INTO GROUP_MEMBER_REFERENCE (GROUP_ID, MEMBER_TYPE, MEMBER_ID, SERVER_ID) VALUES ($1, $2, $3, $4)`,
	}
	...
	QueryCheckGroupNameConflict = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-11",
		Query: `SELECT COUNT(*) as count FROM "GROUP" WHERE NAME = $1 AND OU_ID = $2 AND SERVER_ID = $3`,
	}
	...
	QueryCheckGroupNameConflictForUpdate = dbmodel.DBQuery{
		ID:    "GRQ-GROUP_MGT-12",
		Query: `SELECT COUNT(*) as count FROM "GROUP" WHERE NAME = $1 AND OU_ID = $2 AND GROUP_ID != $3 AND SERVER_ID = $4`,
	}
	...
	QueryGetGroupsByOrganizationUnit = dbmodel.DBQuery{
		...
		Query: `SELECT GROUP_ID, OU_ID, NAME, DESCRIPTION FROM "GROUP" ` +
			`WHERE OU_ID = $1 AND SERVER_ID = $4 ORDER BY NAME LIMIT $2 OFFSET $3`,
	}
)

// buildBulkGroupExistsQuery constructs a query to check which group IDs exist from a list.
func buildBulkGroupExistsQuery(groupIDs []string, serverID string) (dbmodel.DBQuery, []interface{}, error) {
	if len(groupIDs) == 0 {
	...
	baseQuery := "SELECT GROUP_ID FROM \"GROUP\" WHERE SERVER_ID = %s AND GROUP_ID IN (%s)"
