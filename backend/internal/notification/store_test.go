...
func (suite *StoreTestSuite) TestNewNotificationStore() {
	store := newNotificationStore(testServerID)
...
func (suite *StoreTestSuite) TestCreateSender() {
	...
	suite.mockDBClient.EXPECT().Execute(
		...
		string(sender.Type), string(sender.Provider), propsJSON, testServerID,
...
func (suite *StoreTestSuite) TestCreateSender_DBExecuteError() {
	...
	suite.mockDBClient.EXPECT().Execute(queryCreateNotificationSender, sender.Name, sender.ID,
		sender.Description, string(sender.Type), string(sender.Provider), propsJSON, testServerID).Return(
...
func (suite *StoreTestSuite) TestListSenders_WithPropertiesStringAndBytes() {
	...
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Query(queryGetAllNotificationSenders, testServerID).Return(
...
func (suite *StoreTestSuite) TestListSenders_WithError() {
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Query(queryGetAllNotificationSenders, testServerID).
...
func (suite *StoreTestSuite) TestGetSenderByID() {
	...
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Query(queryGetNotificationSenderByID, "s1", testServerID).Return(
		[]map[string]interface{}{row}, nil).Once()
	...
	// not found
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Query(queryGetNotificationSenderByID, "s-x", testServerID).Return(
	...
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Query(queryGetNotificationSenderByID, "s-multi", testServerID).Return(
...
func (suite *StoreTestSuite) TestGetSender_WithError() {
	cases := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T) {
				suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
				suite.mockDBClient.EXPECT().Query(queryGetNotificationSenderByID, "s1", testServerID).
					Return(nil, errors.New("query fail")).Once()
			},
...
func (suite *StoreTestSuite) TestUpdateSender() {
	...
	suite.mockDBClient.EXPECT().Execute(queryUpdateNotificationSender, sender.Name, sender.Description,
		string(sender.Provider), "", "s1", string(sender.Type), testServerID).Return(int64(1), nil).Once()
...
func (suite *StoreTestSuite) TestUpdateSender_WithProperties() {
	...
	suite.NoError(err)
	suite.mockDBClient.EXPECT().Execute(queryUpdateNotificationSender, sender.Name, sender.Description,
		string(sender.Provider), propsJSON, "s1", string(sender.Type), testServerID).Return(int64(1), nil).Once()
		...
		},
		{
			...
			setup: func(t *testing.T) {
				...
				suite.mockDBClient.EXPECT().Execute(queryUpdateNotificationSender, sender.Name,
					sender.Description, string(sender.Provider), "", "s1", string(sender.Type), testServerID).
...
func (suite *StoreTestSuite) TestUpdateSender_ExecuteError() {
	...
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Execute(queryUpdateNotificationSender, sender.Name, sender.Description,
		string(sender.Provider), "", "s1", string(sender.Type), testServerID).Return(int64(0), errors.New("exec fail")).Once()
...
func (suite *StoreTestSuite) TestDeleteSender_NoRows() {
	...
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Execute(queryDeleteNotificationSender, "s1", testServerID).Return(int64(0), nil).Once()
...
func (suite *StoreTestSuite) TestDeleteSender_ExecuteError() {
	suite.mockDBProvider.EXPECT().GetConfigDBClient().Return(suite.mockDBClient, nil).Once()
	suite.mockDBClient.EXPECT().Execute(queryDeleteNotificationSender, "s1", testServerID).Return(
		int64(0), errors.New("exec fail")).Once()
