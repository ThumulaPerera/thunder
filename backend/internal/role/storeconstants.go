...
var (
	...
	queryCreateRole = dbmodel.DBQuery{
		ID:    "RLQ-ROLE_MGT-01",
		Query: `INSERT INTO "ROLE" (ROLE_ID, OU_ID, NAME, DESCRIPTION, SERVER_ID) VALUES ($1, $2, $3, $4, $5)`,
	}

	// queryGetRoleByID retrieves a role by ID.
	queryGetRoleByID = dbmodel.DBQuery{
		ID:    "RLQ-ROLE_MGT-02",
		Query: `SELECT ROLE_ID, OU_ID, NAME, DESCRIPTION FROM "ROLE" WHERE ROLE_ID = $1 AND SERVER_ID = $2`,
	}
	...
	queryGetRoleAssignments = dbmodel.DBQuery{
		...
		Query: `SELECT ASSIGNEE_ID, ASSIGNEE_TYPE FROM ROLE_ASSIGNMENT
			WHERE ROLE_ID = $1 AND SERVER_ID = $4 ORDER BY CREATED_AT LIMIT $2 OFFSET $3`,
	}
	...
	queryGetRoleAssignmentsCount = dbmodel.DBQuery{
		ID:    "RLQ-ROLE_MGT-12",
		Query: `SELECT COUNT(*) as total FROM ROLE_ASSIGNMENT WHERE ROLE_ID = $1 AND SERVER_ID = $2`,
	}
	...
	queryDeleteRoleAssignmentsByIDs = dbmodel.DBQuery{
		...
		Query: `DELETE FROM ROLE_ASSIGNMENT ` +
			`WHERE ROLE_ID = $1 AND ASSIGNEE_TYPE = $2 AND ASSIGNEE_ID = $3 AND SERVER_ID = $4`,
	}
	...
	queryCheckRoleNameExists = dbmodel.DBQuery{
		ID:    "RLQ-ROLE_MGT-14",
		Query: `SELECT COUNT(*) as count FROM "ROLE" WHERE OU_ID = $1 AND NAME = $2 AND SERVER_ID = $3`,
	}
	...
	queryCheckRoleNameExistsExcludingID = dbmodel.DBQuery{
		ID:    "RLQ-ROLE_MGT-15",
		Query: `SELECT COUNT(*) as count FROM "ROLE" WHERE OU_ID = $1 AND NAME = $2 AND ROLE_ID != $3 AND SERVER_ID = $4`,
	}

	// queryCheckRoleExists checks if a role exists by its ID.
	queryCheckRoleExists = dbmodel.DBQuery{
		ID:    "RLQ-ROLE_MGT-16",
		Query: `SELECT COUNT(*) as count FROM "ROLE" WHERE ROLE_ID = $1 AND SERVER_ID = $2`,
	}
)
...
func buildAuthorizedPermissionsQuery(
	...
	baseQuery := `SELECT DISTINCT rp.PERMISSION
		...
		INNER JOIN ROLE_ASSIGNMENT ra ON rp.ROLE_ID = ra.ROLE_ID AND rp.SERVER_ID = $1 AND ra.SERVER_ID = $1
		WHERE rp.SERVER_ID = $1 AND `

	var postgresWhere []string
	var sqliteWhere []string

	// Pre-allocate args slice with estimated capacity
	argsCapacity := 1 + len(groupIDs) + len(requestedPermissions) // +1 for SERVER_ID
	if userID != "" {
		argsCapacity++
	}
	args := make([]interface{}, 0, argsCapacity)
	args = append(args, serverID)
	paramIndex := 2 // Start from $2 since $1 is SERVER_ID
