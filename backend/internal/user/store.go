...
func newUserStore(serverID string) userStoreInterface {
	return &userStore{
		serverID: serverID,
	}
}
...
func (us *userStore) GetUserListCount(filters map[string]interface{}) (int, error) {
	...
	countQuery, args, err := buildUserCountQuery(filters, us.serverID)
	if err != nil {
...
func (us *userStore) GetUserList(limit, offset int, filters map[string]interface{}) ([]User, error) {
	...
	listQuery, args, err := buildUserListQuery(filters, limit, offset, us.serverID)
	if err != nil {
...
func (us *userStore) GetUser(id string) (User, error) {
	...
	results, err := dbClient.Query(QueryGetUserByUserID, id, us.serverID)
	if err != nil {
...
func (us *userStore) UpdateUser(user *User) error {
	...
	rowsAffected, err := dbClient.Execute(
		QueryUpdateUserByUserID, user.ID, user.OrganizationUnit, user.Type, string(attributes), us.serverID)
	if err != nil {
		return fmt.Errorf("failed to execute query: %w", err)
	}
...
func (us *userStore) DeleteUser(id string) error {
	...
	rowsAffected, err := dbClient.Execute(QueryDeleteUserByUserID, id, us.serverID)
	if err != nil {
		return fmt.Errorf("failed to execute query: %w", err)
	}
...
func (us *userStore) IdentifyUser(filters map[string]interface{}) (*string, error) {
	...
	identifyUserQuery, args, err := buildIdentifyQuery(filters, us.serverID)
	if err != nil {
...
func (us *userStore) VerifyUser(id string) (User, []Credential, error) {
	...
	results, err := dbClient.Query(QueryValidateUserWithCredentials, id, us.serverID)
	if err != nil {
...
func (us *userStore) ValidateUserIDs(userIDs []string) ([]string, error) {
	...
	query, args, err := buildBulkUserExistsQuery(userIDs, us.serverID)
	if err != nil {
...
func (us *userStore) GetGroupCountForUser(userID string) (int, error) {
	...
	countResults, err := dbClient.Query(QueryGetGroupCountForUser, userID, us.serverID)
	if err != nil {
...
func (us *userStore) GetUserGroups(userID string, limit, offset int) ([]UserGroup, error) {
	...
	results, err := dbClient.Query(QueryGetGroupsForUser, userID, limit, offset, us.serverID)
	if err != nil {
