...
var (
	...
	queryGetCertificateByID = dbmodel.DBQuery{
		ID:    "CER_MGT-01",
		Query: "SELECT CERT_ID, REF_TYPE, REF_ID, TYPE, VALUE FROM CERTIFICATE WHERE CERT_ID = $1 AND SERVER_ID = $2",
	}
	...
	queryUpdateCertificateByReference = dbmodel.DBQuery{
		ID:    "CER_MGT-05",
		Query: "UPDATE CERTIFICATE SET TYPE = $3, VALUE = $4 WHERE REF_TYPE = $1 AND REF_ID = $2 AND SERVER_ID = $5",
	}
	// queryDeleteCertificateByID deletes a certificate by its ID.
	queryDeleteCertificateByID = dbmodel.DBQuery{
		ID:    "CER_MGT-06",
		Query: "DELETE FROM CERTIFICATE WHERE CERT_ID = $1 AND SERVER_ID = $2",
	}
	...
	queryDeleteCertificateByReference = dbmodel.DBQuery{
		ID:    "CER_MGT-07",
		Query: "DELETE FROM CERTIFICATE WHERE REF_TYPE = $1 AND REF_ID = $2 AND SERVER_ID = $3",
	}
)
