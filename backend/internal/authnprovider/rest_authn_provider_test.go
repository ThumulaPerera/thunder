...
func (suite *RestAuthnProviderTestSuite) TestAuthenticate_Success() {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		suite.Equal("apikey123", r.Header.Get("X-API-KEY"))
	...
	provider := NewRestAuthnProvider(ts.URL, "apikey123", time.Second)
...
func (suite *RestAuthnProviderTestSuite) TestAuthenticate_Failure() {
	...
	provider := NewRestAuthnProvider(ts.URL, "", time.Second)
