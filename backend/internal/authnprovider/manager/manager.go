...
func (m *authnProviderManager) AuthenticateUser(ctx context.Context, identifiers, credentials map[string]interface{},
	...
	if sub, ok := credentials["sub"]; ok {
		...
		storedSub, ok := entityRefToken[userAttributeSub]
		if !ok {
			m.logger.Debug(ctx,
		...
		if systemutils.ConvertInterfaceValueToString(storedSub) !=
		...
		val, ok := identifiers[userAttributeUserID]
		if !ok {
		...
		userIDToken := map[string]interface{}{userAttributeUserID: valStr}
