		...
		[]conflictTestCase{
			{
				...
				setup: func(parent *string) {
					...
					suite.dbClientMock.
						On("Query", withParentQueryID, value, *parent, testServerID).
			...
			},
			{
				...
				setup: func(_ *string) {
					suite.expectDBClient()
					suite.dbClientMock.
						On("Query", withoutParentQueryID, value, testServerID).
...
func (suite *OrganizationUnitStoreTestSuite) runCountQueryScenario(
	queryID interface{}, arg string, serverID string,
	successCount int,
	invoke func() (int, error),
) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryCheckOrganizationUnitHasUsersOrGroups, "ou1", testServerID).
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetOrganizationUnitGroupsCount, "ou1", testServerID).
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(limit, offset int) {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetOrganizationUnitGroupsList, "ou1", limit, offset, testServerID).
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(limit, offset int) {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetOrganizationUnitUsersList, "ou1", limit, offset, testServerID).
					Return([]map[string]interface{}{
						{"user_id": "user1"},
						{"user_id": "user2"},
					}, nil).
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(parent string, limit, offset int) {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetOrganizationUnitChildrenList, parent, limit, offset, testServerID).
					Return([]map[string]interface{}{
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Execute", queryDeleteOrganizationUnit, "ou1", testServerID).
					Return(int64(1), nil).
					Once()
			},
		},
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryCheckOrganizationUnitExists, "ou1", testServerID).
					Return([]map[string]interface{}{{"count": int64(1)}}, nil).
					Once()
			},
			want: true,
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(_ []string) {
				...
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetRootOrganizationUnitByHandle, "root", testServerID).
					Return([]map[string]interface{}{
						makeOUResultRow(rootID, "root", "Root", "desc", nil),
					}, nil).
					Once()
				suite.dbClientMock.
					On("Query", queryGetOrganizationUnitByHandle, "child", rootID, testServerID).
					Return([]map[string]interface{}{
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(id string) {
				...
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetOrganizationUnitByID, id, testServerID).
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(limit, offset int) {
				...
				suite.dbClientMock.
					On("Query", queryGetRootOrganizationUnitList, limit, offset, testServerID).
					Return(rows, nil).
					Once()
			},
			assert: func(ous []OrganizationUnitBasic) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				suite.expectDBClient()
				suite.dbClientMock.
					On("Query", queryGetRootOrganizationUnitListCount, testServerID).
