...
func (s *cacheBackedOUStore) GetOrganizationUnit(ctx context.Context, id string) (OrganizationUnit, error) {
	...
	if cached, ok := s.ouByIDCache.Get(ctx, cacheKey); ok {
		return *cached, nil
	}
...
func (s *cacheBackedOUStore) GetOrganizationUnitByHandle(
	...
	if cached, ok := s.ouByHandleParentCache.Get(ctx, cacheKey); ok {
		return *cached, nil
	}
...
func (s *cacheBackedOUStore) UpdateOrganizationUnit(ctx context.Context, ou OrganizationUnit) error {
	// Pre-fetch old OU to invalidate stale handle+parent cache entry if handle or parent changed.
	s.invalidateHandleParentCache(ctx, ou.ID)
...
func (s *cacheBackedOUStore) DeleteOrganizationUnit(ctx context.Context, id string) error {
	// Pre-fetch OU to invalidate handle+parent cache entry.
	s.invalidateHandleParentCache(ctx, id)
...
// invalidateHandleParentCache looks up the OU (from cache or store) to find its
// handle+parent and invalidate the corresponding cache entry.
func (s *cacheBackedOUStore) invalidateHandleParentCache(ctx context.Context, id string) {
	if id == "" {
		return
	}
	...
	if cached, ok := s.ouByIDCache.Get(ctx, cache.CacheKey{Key: id}); ok && cached != nil {
		...
		if err != nil {
			s.logger.Warn("Failed to fetch OU for handle+parent cache invalidation",
				log.String("ouID", id), log.Error(err))
			return
		}
		ou = &fetched
