...
type CacheBackedEntityStoreTestSuite struct {
	...
	entityIDByClientIDCache        *cachemock.CacheInterfaceMock[*string]
	...
	entityIDByClientIDData         map[string]*string
}
...
func (s *CacheBackedEntityStoreTestSuite) SetupTest() {
	...
	s.entityIDByClientIDData = make(map[string]*string)
	...
	s.entityIDByClientIDCache = cachemock.NewCacheInterfaceMock[*string](s.T())
	...
	setupEntityCacheMock(s.entityIDByClientIDCache, s.entityIDByClientIDData)

	s.entityByIDCache.EXPECT().IsEnabled().Return(true).Maybe()
	s.entityWithCredentialsByIDCache.EXPECT().IsEnabled().Return(true).Maybe()
	s.entityIDByClientIDCache.EXPECT().IsEnabled().Return(true).Maybe()
	...
	s.cachedStore = &cacheBackedEntityStore{
		...
		entityIDByClientIDCache:        s.entityIDByClientIDCache,
		store:                          s.mockStore,
...
func (s *CacheBackedEntityStoreTestSuite) TestCreateEntity_CachesEntityByID() {
	...
	cachedID, ok := s.entityIDByClientIDCache.Get(context.Background(),
		cache.CacheKey{Key: "client-1"})
	s.True(ok)
...
func (s *CacheBackedEntityStoreTestSuite) TestCreateEntity_StoreError_DoesNotCache() {
	...
	_, ok = s.entityIDByClientIDCache.Get(context.Background(),
		cache.CacheKey{Key: "client-1"})
	s.False(ok)
}
...
func (s *CacheBackedEntityStoreTestSuite) TestIdentifyEntity_CacheHit_ClientIDFilter() {
	entityID := testEntityID
	s.entityIDByClientIDData["client-1"] = &entityID
...
func (s *CacheBackedEntityStoreTestSuite) TestIdentifyEntity_CacheMiss_ClientIDFilter_PopulatesCache() {
	entityID := testEntityID
	filters := map[string]interface{}{"clientId": "client-1"}
	s.mockStore.On("IdentifyEntity", mock.Anything, filters).Return(&entityID, nil).Once()
...
func (s *CacheBackedEntityStoreTestSuite) TestIdentifyEntity_NonClientIDFilter_BypassesCache() {
	entityID := testEntityID
...
func (s *CacheBackedEntityStoreTestSuite) TestIdentifyEntity_StoreReturnsNil_DoesNotPopulateCache() {
	...
	_, ok := s.entityIDByClientIDCache.Get(context.Background(),
		cache.CacheKey{Key: "unknown"})
	s.False(ok)
}

// ClientID cache invalidation tests

func (s *CacheBackedEntityStoreTestSuite) TestDeleteEntity_InvalidatesClientIDCache() {
	entity := s.makeEntity(testEntityID, "client-1")
	s.entityByIDData[entity.ID] = &entity
	entityID := entity.ID
	s.entityIDByClientIDData["client-1"] = &entityID
...
func (s *CacheBackedEntityStoreTestSuite) TestUpdateEntity_InvalidatesAndRewarmsClientIDCache() {
	entity := s.makeEntity(testEntityID, "client-1")
	s.entityByIDData[entity.ID] = &entity
	entityID := entity.ID
	s.entityIDByClientIDData["client-1"] = &entityID
...
func (s *CacheBackedEntityStoreTestSuite) TestUpdateSystemAttributes_InvalidatesClientIDCache() {
	entity := s.makeEntity(testEntityID, "client-1")
	s.entityByIDData[entity.ID] = &entity
	entityID := entity.ID
	s.entityIDByClientIDData["client-1"] = &entityID
...
func (s *CacheBackedEntityStoreTestSuite) TestDeleteEntity_InvalidatesClientIDCache_CacheMiss() {
	entity := s.makeEntity(testEntityID, "client-1")
	entityID := entity.ID
	s.entityIDByClientIDData["client-1"] = &entityID

	// Entity is NOT in entityByIDCache — invalidateClientIDCache must fall back to the store.
	s.mockStore.On("DeleteEntity", mock.Anything, entity.ID).Return(nil).Once()
	s.mockStore.On("GetEntity", mock.Anything, entity.ID).Return(entity, nil).Once()
...
func (s *CacheBackedEntityStoreTestSuite) TestUpdateCredentials_DoesNotInvalidateClientIDCache() {
	entity := s.makeEntity(testEntityID, "client-1")
	s.entityByIDData[entity.ID] = &entity
	entityID := entity.ID
	s.entityIDByClientIDData["client-1"] = &entityID
