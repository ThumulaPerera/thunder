...
func newApplicationStore(serverID string) applicationStoreInterface {
...
func (st *applicationStore) CreateApplication(app model.ApplicationProcessedDTO) error {
	...
	if len(app.InboundAuthConfig) > 0 {
		queries = append(queries, createOAuthAppQuery(&app, QueryCreateOAuthApplication, st.serverID))
	}
...
func (st *applicationStore) GetTotalApplicationCount() (int, error) {
	...
	results, err := dbClient.Query(QueryGetApplicationCount, st.serverID)
	if err != nil {
...
func (st *applicationStore) GetApplicationList() ([]model.BasicApplicationDTO, error) {
	...
	results, err := dbClient.Query(QueryGetApplicationList, st.serverID)
	if err != nil {
...
func (st *applicationStore) GetOAuthApplication(clientID string) (*model.OAuthAppConfigProcessedDTO, error) {
	...
	results, err := dbClient.Query(QueryGetOAuthApplicationByClientID, clientID, st.serverID)
	if err != nil {
		return nil, err
	}
	if len(results) == 0 {
...
func (st *applicationStore) GetApplicationByID(id string) (*model.ApplicationProcessedDTO, error) {
	return st.getApplicationByQuery(QueryGetApplicationByAppID, id, st.serverID)
}
...
func (st *applicationStore) GetApplicationByName(name string) (*model.ApplicationProcessedDTO, error) {
	return st.getApplicationByQuery(QueryGetApplicationByName, name, st.serverID)
}
	...
	queries := []func(tx dbmodel.TxInterface) error{
		func(tx dbmodel.TxInterface) error {
			...
			_, err := tx.Exec(QueryUpdateApplicationByAppID.Query, updatedApp.ID, updatedApp.Name,
				...
				isRegistrationEnabledStr, brandingID, jsonDataBytes, st.serverID)
			return err
		},
	}
	// TODO: Need to refactor when supporting other/multiple inbound auth types.
	if len(updatedApp.InboundAuthConfig) > 0 && len(existingApp.InboundAuthConfig) > 0 {
		queries = append(queries, createOAuthAppQuery(updatedApp, QueryUpdateOAuthApplicationByAppID, st.serverID))
		...
		queries = append(queries, deleteOAuthAppQuery(clientID, st.serverID))
	} else if len(updatedApp.InboundAuthConfig) > 0 {
		queries = append(queries, createOAuthAppQuery(updatedApp, QueryCreateOAuthApplication, st.serverID))
	}
...
func (st *applicationStore) DeleteApplication(id string) error {
	...
	_, err = dbClient.Execute(QueryDeleteApplicationByAppID, id, st.serverID)
	if err != nil {
		logger.Error("Failed to execute query", log.Error(err))
...
func createOAuthAppQuery(app *model.ApplicationProcessedDTO,
	oauthAppMgtQuery dbmodel.DBQuery, serverID string) func(tx dbmodel.TxInterface) error {
	...
	return func(tx dbmodel.TxInterface) error {
		_, err := tx.Exec(oauthAppMgtQuery.Query, app.ID, clientID, clientSecret, oauthConfigJSON, serverID)
		return err
	}
}

// deleteOAuthAppQuery creates a query function for deleting an OAuth application by client ID.
func deleteOAuthAppQuery(clientID string, serverID string) func(tx dbmodel.TxInterface) error {
	return func(tx dbmodel.TxInterface) error {
		_, err := tx.Exec(QueryDeleteOAuthApplicationByClientID.Query, clientID, serverID)
		return err
	}
}
