...
const (
	basicAuthExternalLoggerComponentName = "BasicAuthExecutorExternal"
	inputDataTypePasswordExternal        = "PASSWORD_INPUT"
	ExecutorNameBasicAuthExternal        = "BasicAuthExecutorExternal"
)

// basicAuthExecutorExternal implements the ExecutorInterface for basic authentication with external service.
type basicAuthExecutorExternal struct {
	core.ExecutorInterface
	identifyingExecutorExternalInterface
	credsAuthSvc     authncreds.CredentialsAuthnServiceExternalInterface
	observabilitySvc observability.ObservabilityServiceInterface
	logger           *log.Logger
}

var _ core.ExecutorInterface = (*basicAuthExecutorExternal)(nil)
var _ identifyingExecutorExternalInterface = (*basicAuthExecutorExternal)(nil)

// newBasicAuthExecutorExternal creates a new instance of BasicAuthExecutorExternal.
func newBasicAuthExecutorExternal(
	flowFactory core.FlowFactoryInterface,
	externalSvc externalsvc.ExternalSvcInterface,
	credsAuthSvc authncreds.CredentialsAuthnServiceExternalInterface,
	observabilitySvc observability.ObservabilityServiceInterface,
) *basicAuthExecutorExternal {
	defaultInputs := []common.Input{
		...
		},
		{
			...
			Type:       inputDataTypePasswordExternal,
			Required:   true,
		},
	}

	logger := log.GetLogger().With(log.String(log.LoggerKeyComponentName, basicAuthExternalLoggerComponentName),
		log.String(log.LoggerKeyExecutorName, ExecutorNameBasicAuthExternal))

	identifyExec := newIdentifyingExecutorExternal(ExecutorNameBasicAuthExternal, defaultInputs, []common.Input{},
		flowFactory, externalSvc)
	base := flowFactory.CreateExecutor(ExecutorNameBasicAuthExternal, common.ExecutorTypeAuthentication,
		defaultInputs, []common.Input{})

	return &basicAuthExecutorExternal{
		ExecutorInterface:                    base,
		identifyingExecutorExternalInterface: identifyExec,
...
func (b *basicAuthExecutorExternal) Execute(ctx *core.NodeContext) (*common.ExecutorResponse, error) {
...
func (b *basicAuthExecutorExternal) getAuthenticatedUser(ctx *core.NodeContext,
